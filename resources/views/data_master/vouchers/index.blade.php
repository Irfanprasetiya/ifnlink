...
@section('title', 'Data Produk Toko')
...
@section('container')
    <div class="max-w-7xl mx-auto space-y-4 sm:space-y-6 pb-32 relative mt-4 sm:mt-7">
        ...
        {{-- ========== PAGE HEADER & ACTIONS ========== --}}
        <div
            class="bg-white rounded-xl sm:rounded-2xl shadow-sm sm:shadow-soft border border-slate-200/80 p-3.5 sm:p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4 sm:gap-5">

            <div class="flex justify-between items-center lg:block">
                <h1 class="text-base sm:text-xl font-extrabold text-slate-800 tracking-tight">Data Master Produk</h1>
                <p class="text-[11px] sm:text-sm text-slate-500 sm:mt-1 hidden sm:block">Kelola harga beli, harga jual, dan
                    kategori produk</p>
            </div>
                ...
                <form method="GET" class="flex flex-col sm:flex-row flex-1 sm:flex-none gap-2">
                    ...
                    <div class="flex gap-2">
                        <select name="kategori_id"
                            class="w-full sm:w-40 bg-slate-50 border border-slate-200 rounded-lg sm:rounded-xl px-2.5 sm:px-3 py-2 sm:py-2.5 text-xs sm:text-sm font-medium text-slate-700 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/30">
                        ...
                        <button type="submit"
                            class="bg-slate-900 hover:bg-slate-800 text-white px-3.5 sm:px-5 py-2 sm:py-2.5 rounded-lg sm:rounded-xl text-xs sm:text-sm font-bold transition-all active:scale-95 shrink-0">
                            Filter
                        </button>
                    </div>
                </form>
        ...
        <div
            ...
            <div class="hidden lg:flex p-4 border-b border-slate-100 items-center gap-2 bg-slate-50/50">
                ...
                <h3 class="font-bold text-slate-800 text-sm">Daftar Produk</h3>
            </div>
                        ...
                        @forelse ($vouchers as $voucher)
                            <tr class="hover:bg-slate-50/60 transition-colors">
                                ...
                                <td class="px-5 py-3.5 text-slate-600 font-medium">
                                    <span
                                        class="bg-slate-100 text-slate-700 px-2.5 py-1 rounded-md text-xs border border-slate-200">{{ $voucher->kategori->nama_kategori ?? '-' }}</span>
                                </td>
                                <td class="px-5 py-3.5 text-right font-medium text-slate-600">Rp
                                    {{ number_format($voucher->harga_beli, 0, ',', '.') }}</td>
                                <td class="px-5 py-3.5 text-right font-bold text-emerald-600">Rp
                                    {{ number_format($voucher->harga_jual, 0, ',', '.') }}</td>
                ...
                @forelse ($vouchers as $voucher)
                    ...
                    <div class="bg-white border border-slate-200 rounded-xl p-3.5 shadow-sm relative">
                            ...
                            <div class="flex items-start gap-2.5 pr-2">
                                ...
                                <div>
                                    <h3 class="font-bold text-slate-800 text-sm leading-tight mb-1">
                                        {{ $voucher->nama_produk }}</h3>
                                    <span
                                        class="text-[9px] font-bold uppercase tracking-wider text-slate-500 bg-slate-50 px-1.5 py-0.5 rounded border border-slate-200">{{ $voucher->kategori->nama_kategori ?? '-' }}</span>
                                </div>
                            </div>
                        ...
                        <div class="flex items-center justify-between pt-2 border-t border-dashed border-slate-200">
                            <div>
                                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Harga Beli
                                </p>
                                <p class="font-semibold text-slate-600 text-xs">Rp
                                    {{ number_format($voucher->harga_beli, 0, ',', '.') }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Harga Jual
                                </p>
                                <p class="font-black text-emerald-600 text-sm">Rp
                                    {{ number_format($voucher->harga_jual, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                @empty

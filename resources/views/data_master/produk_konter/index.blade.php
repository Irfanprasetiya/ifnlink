...
@section('container')
    <div class="max-w-7xl mx-auto space-y-4 sm:space-y-6 pb-32 relative mt-4 sm:mt-7">
        ...
        {{-- ========== PAGE HEADER & ACTIONS ========== --}}
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-sm sm:shadow-soft border border-slate-200/80 p-3.5 sm:p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-3 sm:gap-5">
            
            <div class="flex justify-between items-center lg:block">
                <h1 class="text-base sm:text-xl font-extrabold text-slate-800 tracking-tight">Data Produk Konter</h1>
                <p class="text-[11px] sm:text-sm text-slate-500 sm:mt-1 hidden sm:block">Kelola stok dan distribusi voucher per cabang</p>
            </div>

            <div class="flex flex-col sm:flex-row w-full lg:w-auto gap-2 sm:gap-3">
                {{-- Form Filter --}}
                <form method="GET" class="flex flex-1 sm:flex-none gap-2">
                    <select name="cabang_id" class="w-full sm:w-48 bg-slate-50 border border-slate-200 rounded-lg sm:rounded-xl px-2.5 sm:px-3 py-1.5 sm:py-2.5 text-xs sm:text-sm font-medium text-slate-700 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/30">
                    ...
                    <button type="submit" class="bg-slate-900 hover:bg-slate-800 text-white px-3.5 sm:px-5 py-1.5 sm:py-2.5 rounded-lg sm:rounded-xl text-xs sm:text-sm font-bold transition-all active:scale-95 shrink-0">
                        Filter
                    </button>
                </form>

                {{-- Tombol Tambah --}}
                <button data-modal-target="create-modal" data-modal-toggle="create-modal" class="w-full sm:w-auto justify-center bg-blue-600 hover:bg-blue-700 text-white px-3.5 sm:px-5 py-2 sm:py-2.5 rounded-lg sm:rounded-xl text-xs sm:text-sm font-bold flex items-center gap-1.5 transition-all active:scale-95 shadow-sm">
                        ...
                        @forelse ($produkKonters as $item)
                            <tr class="hover:bg-slate-50/60 transition-colors">
                                ...
                                <td class="px-5 py-3.5 text-slate-600 font-medium">
                                    <span class="bg-slate-100 text-slate-700 px-2.5 py-1 rounded-md text-xs border border-slate-200">{{ $item->cabang->nama_cabang }}</span>
                                </td>
                                <td class="px-5 py-3.5 text-center">
                                    <span class="font-black text-blue-600 bg-blue-50 px-3 py-1 rounded-md">{{ $item->stok }}</span>
                                </td>
                                <td class="px-5 py-3.5 text-slate-500 text-xs truncate max-w-[200px]">{{ $item->keterangan ?: '-' }}</td>
                                <td class="px-5 py-3.5 flex justify-center gap-2">
                                    <button data-modal-toggle="edit-modal-{{ $item->id }}" class="inline-flex items-center justify-center px-3 py-1.5 bg-white border border-amber-300 text-amber-600 rounded-lg text-xs font-bold hover:bg-amber-50 transition-colors shadow-sm active:scale-95">
                                        Edit
                                    </button>
                                    <form method="POST" action="{{ route('data_master.produk_konter.destroy', $item->id) }}" onsubmit="return confirm('Yakin ingin menghapus produk ini dari konter?')">
                                        @csrf @method('DELETE')
                    ...
                    <div class="bg-white border border-slate-200 rounded-xl p-3.5 shadow-sm relative">
                        ...
                        <div class="flex items-center justify-between mb-3 pb-2.5 border-b border-slate-100">
                            <div class="flex items-center gap-2">
                                <span class="bg-slate-800 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">#{{ $noMobile++ }}</span>
                                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-500">{{ $item->cabang->nama_cabang }}</span>
                            </div>
                            
                            {{-- Action Mini (Mobile) --}}
                            <div class="flex items-center gap-1.5">
                                <button data-modal-toggle="edit-modal-{{ $item->id }}" class="p-1.5 bg-amber-50 text-amber-600 rounded-md border border-amber-200 active:scale-90 transition-transform">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                </button>
                                <form method="POST" action="{{ route('data_master.produk_konter.destroy', $item->id) }}" onsubmit="return confirm('Yakin ingin menghapus?')" class="m-0 p-0">
                                    @csrf @method('DELETE')
                        ...
                        {{-- Body: Nama & Ket --}}
                        <div class="mb-3">
                            <p class="font-bold text-slate-800 text-sm leading-tight">{{ $item->voucher->nama_produk }}</p>
                            @if($item->keterangan)
                        ...
                        <div class="flex items-center justify-between pt-2.5 border-t border-dashed border-slate-200">
                            ...
                            <span class="font-black text-blue-600 text-base bg-blue-50 px-2 py-0.5 rounded border border-blue-100">{{ $item->stok }}</span>
                        </div>
                    </div>
                @empty
    ...
    <form method="POST" action="{{ route('data_master.produk_konter.store') }}">
        @csrf
        <div id="create-modal" tabindex="-1" aria-hidden="true" class="hidden fixed inset-0 z-[60] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden transform transition-all">
                ...
                <div class="p-5 space-y-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wide">Pilih Voucher</label>
                        <select name="voucher_id" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/30">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wide">Pilih Cabang</label>
                        <select name="cabang_id" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/30">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wide">Jumlah Stok</label>
                        <input type="number" name="stok" required placeholder="0" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/30">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wide">Keterangan (Opsional)</label>
                        <input type="text" name="keterangan" placeholder="Contoh: Stok awal bulan" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/30">
                    </div>
                </div>
    ...
    @foreach ($produkKonters as $item)
        <form method="POST" action="{{ route('data_master.produk_konter.update', $item->id) }}">
            @csrf @method('PUT')
            <div id="edit-modal-{{ $item->id }}" tabindex="-1" aria-hidden="true" class="hidden fixed inset-0 z-[60] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
                <div class="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden transform transition-all">
                    <div class="px-5 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        <h3 class="text-base font-extrabold text-slate-800">Edit Produk Konter</h3>
                        <button type="button" data-modal-toggle="edit-modal-{{ $item->id }}" class="text-slate-400 hover:text-rose-500 transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                        </button>
                    </div>
                    <div class="p-5 space-y-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wide">Voucher</label>
                            <select name="voucher_id" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/30">
                                @foreach ($vouchers as $voucher)
                                    <option value="{{ $voucher->id }}" {{ $voucher->id == $item->voucher_id ? 'selected' : '' }}>
                                        {{ $voucher->nama_produk }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wide">Cabang</label>
                            <select name="cabang_id" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/30">
                                @foreach ($cabangs as $cabang)
                                    <option value="{{ $cabang->id }}" {{ $cabang->id == $item->cabang_id ? 'selected' : '' }}>
                                        {{ $cabang->nama_cabang }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wide">Jumlah Stok</label>
                            <input type="number" name="stok" value="{{ $item->stok }}" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/30">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wide">Keterangan</label>
                            <input type="text" name="keterangan" value="{{ $item->keterangan }}" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/30">
                        </div>
                    </div>
                    <div class="px-5 py-4 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-2">
                        <button type="button" data-modal-toggle="edit-modal-{{ $item->id }}" class="px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 transition-colors">Batal</button>

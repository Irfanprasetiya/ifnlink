{{-- resources/views/laporan_bank/rekap.blade.php --}}

@extends('layouts.app')

@section('title', 'Rekap Laporan Bank')

@section('container')
    <div class="w-full mx-auto mt-6 px-4 py-6 max-w-6xl">

        {{-- Header --}}
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Rekap Laporan Bank</h1>
                <p class="text-sm text-gray-500 mt-1">
                    {{ $user->name ?? '-' }} · {{ $cabang->nama_cabang ?? '-' }} ·
                    {{ \Carbon\Carbon::parse($tanggal)->translatedFormat('d F Y') }}
                </p>
            </div>

            <div class="flex gap-2">
                <a href="{{ route('laporan-bank.rekap.pdf', request()->only(['tanggal', 'cabang_id', 'user_id'])) }}"
                ...
                <a href="{{ route('laporan-bank.rekap.excel', request()->only(['tanggal', 'cabang_id', 'user_id'])) }}"
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                ...
                <p class="text-lg font-semibold text-blue-600">Rp {{ number_format($saldoAwalKas ?? 0, 0, ',', '.') }}</p>
            </div>
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                <p class="text-xs text-gray-500 mb-1">Tambahan Kas</p>
                <p class="text-lg font-semibold text-amber-600">Rp {{ number_format($tambahanKas ?? 0, 0, ',', '.') }}</p>
            </div>
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                <p class="text-xs text-gray-500 mb-1">Pengurangan Kas</p>
                <p class="text-lg font-semibold text-red-600">Rp {{ number_format($penguranganKas ?? 0, 0, ',', '.') }}</p>
            </div>
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                <p class="text-xs text-gray-500 mb-1">Total Transfer</p>
                <p class="text-lg font-semibold text-amber-600">Rp {{ number_format($totalTransfer ?? 0, 0, ',', '.') }}</p>
            </div>
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                <p class="text-xs text-gray-500 mb-1">Total Tarik Tunai</p>
                <p class="text-lg font-semibold text-amber-600">Rp {{ number_format($totalTarikTunai ?? 0, 0, ',', '.') }}
                </p>
            </div>
            <div class="bg-blue-50 rounded-xl border border-blue-100 shadow-sm p-4">
                <p class="text-xs text-blue-600 mb-1">Saldo Akhir Kas</p>
                <p class="text-lg font-bold text-blue-700">Rp {{ number_format($saldoAkhirKas ?? 0, 0, ',', '.') }}</p>
            </div>
        </div>
    ...
    <script>
        ...
        new Chart(ctxBar, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Jumlah (Rp)',
                    data: [{{ $totalTransfer ?? 0 }}, {{ $totalTarikTunai ?? 0 }}],

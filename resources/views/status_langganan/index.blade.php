        ...
        <div class="bg-white dark:bg-gray-800 rounded-3xl border shadow-sm overflow-hidden">
            ...
            @if ($pembayarans->count() > 0)
                ...
                <div class="overflow-x-auto">
                    ...
                    <table class="w-full text-left whitespace-nowrap" id="tabel-pembayaran">
                            ...
                            @foreach ($pembayarans as $p)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition"
                                    ...
                                    <td class="py-4 px-6 font-bold text-gray-900 dark:text-white">
                                        ...
                                        #{{ $p->order_id ?? 'INV-' . str_pad($p->id, 5, '0', STR_PAD_LEFT) }}
                                    ...
                                    <td class="py-4 px-6 text-center status-cell">

                                        {{-- {!! $this->renderStatusBadge($p->status ?? '') !!} --}}

                                        {{-- diganti JS, fallback server-side di bawah --}}
                                        ...
                                        @elseif($p->status == 'pending')
                                            <span
                                                class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-700 border border-amber-200 animate-pulse">⏳
    ...
    @if ($hasPendingPayment)
        <script>
            (function() {
                ...
                const URL_CEK = @json(route('status.langganan.cek'));
                ...
                function badgeHtml(status) {
                    ...
                    if (status === 'confirmed') {
                        ...
                        return `<span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">✅ Lunas</span>`;

                    }

                    if (status === 'pending') {

                        return `<span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-700 border border-amber-200 animate-pulse">⏳ Menunggu</span>`;

                    }

                    return `<span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-red-100 text-red-700 border border-red-200">❌ ${status.charAt(0).toUpperCase() + status.slice(1)}</span>`;
                ...
                async function poll() {
                    ...
                    try {
                        ...
                        // Update status tiap baris

                        (json.data || []).forEach(item => {

                            const row = document.querySelector(`#tabel-pembayaran tr[data-id="${item.id}"]`);
                            ...
                            if (cell) {
                                ...
                                cell.innerHTML = badgeHtml(item.status);
                        ...
                        // Jika sudah tidak ada pending → stop + refresh sekali
                        ...
                        if (!json.has_pending) {
                            ...
                            // Optional: reload agar info langganan ikut update

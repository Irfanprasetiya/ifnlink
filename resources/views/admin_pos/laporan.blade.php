...
@section('container')
    <div class="max-w-7xl mx-auto space-y-4 sm:space-y-6 pb-32 relative mt-4 sm:mt-7">
        ...
        @if (session('success'))
            <div
                class="mb-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-3 text-sm font-medium">
                {{ session('success') }}
            </div>
        @endif
        ...
        {{-- HEADER & FILTER --}}
        <div
            class="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-slate-200/80 p-3.5 sm:p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-[11px] sm:text-sm text-slate-500 mt-1">
                    Periode: <span class="font-bold">{{ \Carbon\Carbon::parse($startDate)->translatedFormat('d M Y') }} -
                        {{ \Carbon\Carbon::parse($endDate)->translatedFormat('d M Y') }}</span>
                </p>
            </div>
            <form method="GET" class="flex flex-wrap gap-2">
                <input type="date" name="start_date" value="{{ $startDate }}"
                    class="bg-slate-50 border rounded-lg px-3 py-2 text-sm">
                ...
                <button type="submit"
                    class="bg-slate-900 text-white px-4 py-2 rounded-lg text-sm font-bold">Filter</button>
            </form>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left whitespace-nowrap">
                    ...
                    <tbody class="divide-y text-sm">
                            ...
                            @foreach ($p->details as $detail)
                                <tr class="hover:bg-slate-50/50">
                                    ...
                                    <td class="px-4 py-3">
                                        @if ($loop->first)
                                            <span
                                                class="font-mono text-xs font-bold bg-slate-100 px-2 py-1 rounded">{{ $p->kode_transaksi ?? 'TRX-' . str_pad($p->id, 5, '0', STR_PAD_LEFT) }}</span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 text-xs text-slate-500">
                                        {{ $loop->first ? $p->created_at->translatedFormat('d M Y H:i') : '' }}</td>
                                    <td class="px-4 py-3 font-bold text-slate-800">
                                        {{ $detail->voucher->nama_produk ?? '-' }}</td>
                                    <td class="px-4 py-3 text-xs text-slate-500">
                                        {{ $detail->voucher->kategori->nama_kategori ?? '-' }}</td>
                                    <td class="px-4 py-3">{{ $p->cabang->nama_cabang ?? '-' }}</td>
                                    <td class="px-4 py-3">{{ $p->user->name ?? '-' }}</td>
                                    <td class="px-4 py-3 text-right">Rp {{ number_format($p->total_harga, 0, ',', '.') }}
                                    </td>
                                    <td class="px-4 py-3 text-right text-rose-500 text-xs font-bold">
                                        @if ($p->diskon > 0 && $loop->first)
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        @if ($loop->first)
                                            <span class="bg-emerald-50 text-emerald-700 font-extrabold px-2 py-1 rounded">Rp
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        @if ($loop->first)
                                            <div class="flex justify-center gap-1.5">
                                                ...
                                                <form action="{{ route('admin.pos.destroy', $p->id) }}" method="POST"
                                                    onsubmit="return confirm('Hapus?')">
                                                    @csrf @method('DELETE')
                                                    <button type="submit" class="p-1.5 bg-rose-50 text-rose-600 rounded-lg"
                                                        title="Hapus">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        @empty
                            <tr>
                                <td colspan="11" class="text-center py-16 text-slate-500">Belum ada data</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            @forelse($penjualans as $p)
                <div class="bg-white border rounded-xl p-4">
                        ...
                        @foreach ($p->details as $d)
                            <div class="flex justify-between text-sm">
                                <div>
                                    ...
                                    <p class="text-xs text-slate-400">{{ $d->voucher->kategori->nama_kategori ?? '-' }} |
                                        {{ $d->qty }}x</p>
                                </div>
                    ...
                    <div class="flex gap-2 mt-3">
                        <button onclick="openEditModal({{ $p->id }})"
                            class="flex-1 bg-amber-50 text-amber-600 py-2 rounded-lg text-sm font-bold">Edit</button>
                        <form action="{{ route('admin.pos.destroy', $p->id) }}" method="POST"
                            onsubmit="return confirm('Hapus?')" class="flex-1">
                            @csrf @method('DELETE')
                            <button type="submit"
                                class="w-full bg-rose-50 text-rose-600 py-2 rounded-lg text-sm font-bold">Hapus</button>
                        </form>
                    </div>
                </div>
            @empty
    ...
    <div id="edit-modal" class="hidden fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 p-4">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[85vh] flex flex-col overflow-hidden">
            ...
            <form id="edit-form" method="POST" onsubmit="return submitEdit(event)"
                ...
                <div class="px-5 py-4 border-t bg-slate-50">
                    ...
                    <div>
                        ...
                        <input type="text" id="edit-diskon" name="diskon"
                            oninput="formatDiskonInput(this); updateEditTotal();"
                            class="w-full bg-white border rounded-lg px-3 py-2 text-sm font-bold text-rose-500 text-right"
                            placeholder="0">
                    </div>
                    ...
                    <button type="submit" id="btn-simpan"
                        class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 rounded-xl mt-3 flex items-center justify-center gap-2">
                        <svg id="btn-simpan-spinner" class="hidden animate-spin h-5 w-5"
    ...
    <script>
        ...
        function openEditModal(id) {
            ...
            fetch(`/admin/pos/${id}/edit`)
                ...
                .then(data => {
                    ...
                    document.getElementById('edit-diskon').value = data.diskon > 0 ? Number(data.diskon).toLocaleString(
                        'id-ID') : '';
        ...
        function renderEditItems() {
            ...
            <div class="bg-white border border-slate-200 rounded-xl p-3 shadow-sm">
                ...
                {{-- ✅ Dropdown searchable untuk UBAH produk --}}
                ...
                <p class="text-xs text-slate-400 mt-1">
                    Harga: Rp <span class="harga-text">${Number(item.harga_satuan).toLocaleString()}</span>
                </p>

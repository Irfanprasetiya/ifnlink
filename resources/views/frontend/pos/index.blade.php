...
@section('container')
        ...
        <div
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 overflow-y-auto" id="produk-list">
                @forelse($produks as $produk)
                    <button type="button"
                        class="produk-item group flex flex-col justify-between border border-slate-200 bg-white rounded-xl p-3 sm:p-4 text-left transition-all hover:border-blue-400 hover:shadow-[0_8px_15px_-3px_rgba(37,99,235,0.12)] active:scale-95"
                        data-id="{{ $produk->voucher_id }}" data-nama="{{ $produk->voucher->nama_produk }}"
                        data-harga="{{ $produk->voucher->harga_jual }}" data-stok="{{ $produk->stok }}">

                        <div class="w-full">
                            <p
                                class="font-bold text-sm text-slate-800 leading-snug group-hover:text-blue-700 transition-colors line-clamp-2">
                                {{ $produk->voucher->nama_produk }}
                            </p>
                        </div>
                        <div class="mt-3 w-full">
                            <p class="text-blue-600 font-extrabold text-sm sm:text-base">
                                Rp {{ number_format($produk->voucher->harga_jual, 0, ',', '.') }}
                            </p>
                            <div class="flex items-center gap-1.5 mt-1">
                                <span
                                    class="w-1.5 h-1.5 rounded-full {{ $produk->stok > 5 ? 'bg-emerald-500' : 'bg-rose-500' }}"></span>
                                <p class="text-xs font-medium text-slate-500">Sisa: {{ $produk->stok }}</p>
                            </div>
                        </div>
                    </button>
                @empty
        ...
        <div id="keranjang-section" class="lg:sticky lg:top-20 h-fit flex flex-col">
            <div class="bg-white rounded-2xl shadow-soft border border-slate-200/80 p-4 sm:p-5 flex flex-col">
                ...
                <div class="border-t border-slate-100 pt-4 space-y-4">
                    ...
                    <div>
                        <label
                            class="text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1.5 block px-1">Potongan
                            Diskon</label>
                        <div class="relative">
    ...
    <button type="button" id="fab-cart"
        class="lg:hidden fixed bottom-24 right-4 z-40 bg-slate-900 text-white p-4 rounded-full shadow-lg flex items-center justify-center gap-2 transition-transform transform translate-y-20 opacity-0">
    ...
    {{-- ✅ Modal sukses dengan Cetak Struk --}}
    ...
    <script>
        ...
        function showCartAlert(msg) {
            ...
            const alertText = document.getElementById('cart-alert-text');
        ...
        document.querySelectorAll('.produk-item').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (existing) {
                    ...
                    if (stok < 1) {
                        showCartAlert(`Maaf, stok ${nama} kosong.`);
                        return;
                    }
                    cart.push({
                        id,
        ...
        function getTotalSetelahDiskon() {
            ...
            return totalSetelahDiskon > 0 ? totalSetelahDiskon : 0;
        }
        ...
        function hitungKembalian() {
            ...
            if (bayar > 0 && kembalian >= 0) {
                ...
                elKembalian.classList.replace('text-rose-500', 'text-emerald-500');
            } else if (bayar > 0 && kembalian < 0) {
                elKembalian.textContent = 'Uang Kurang!';
                elKembalian.classList.replace('text-emerald-500', 'text-rose-500');
            } else {
        ...
        document.getElementById('btn-uang-pas').addEventListener('click', () => {
            ...
            const totalSetelahDiskon = getTotalSetelahDiskon();
            document.getElementById('bayar').value = totalSetelahDiskon.toLocaleString('id-ID');
            hitungKembalian();
        });
        ...
        document.getElementById('search-produk').addEventListener('input', function() {
            ...
            document.querySelectorAll('.produk-item').forEach(btn => {
                ...
                btn.style.display = nama.includes(keyword) ? '' : 'none';
            });
        });
